...
/* 		$com = "SELECT id_post, id_user,id_comment, body,timestamp FROM comment WHERE id_post = ". $row["id_post"].' ORDER BY timestamp DESC';

		$res_com = $conn->query($com);
		while($row = $res_com->fetch()){
			echo "id_post:" . $row["id_post"]. " userid: " . $row["id_user"]. "body " . $row["body"]. "at".$row["timestamp"];
		} */
		echo "</br></br>";
    }
